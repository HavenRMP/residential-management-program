...
public class ViviendasController : ControllerBase
{
    ...
    [Authorize]
    [HttpPost("~/api/codigos/vivienda/redimir")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    ...
    public async Task<IActionResult> RedimirCodigoVivienda([FromBody] RedimirCodigoRequestDto dto)
    {
        if (!ModelState.IsValid)
            return BadRequest(ModelState);
        ...
        try
        {
            var result = await _supabaseService.RedimirCodigoViviendaAsync(dto.Codigo, userId, userId);
            return Ok(result);
        }
        catch (SupabaseRpcException ex)
        {
